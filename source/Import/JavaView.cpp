...
GReturnValue CJavaView::Import(CGCLCInput *pInput,
                               QTextEdit *out /*FILE *hOutput*/) {
  m_pInput = pInput;
  m_Output = out;
...
GReturnValue CJavaView::ReadPointSet() {
  ...
  do {
    ...
    if (!strcmp(word, JVTagName[_jv_pointset][1])) {
      ...
      while (pPoint != NULL) {
        i++;
        if (!((pPoint->r == 0) && (pPoint->g == 0) && (pPoint->b == 0))) {
          sprintf(m_sOutput, "color %i %i %i", (int)(pPoint->r),
                  (int)(pPoint->g), (int)(pPoint->b));
          Output(m_sOutput);
        }

        sprintf(m_sOutput, "point P%i_%i %3.2f %3.2f", m_iGeometryIndex, i,
                pPoint->x, pPoint->y);
        Output(m_sOutput);
        //				sprintf(m_sOutput,"mark
        // P%i_%i",m_iGeometryIndex, i);
        //				sprintf(m_sOutput,"cmark
        // P%i_%i",m_iGeometryIndex, i);
        //				Output(m_sOutput);
...
GReturnValue CJavaView::ReadFaceSet() {
  ...
  do {
    ...
    if (!strcmp(word, JVTagName[_jv_faceset][1])) {
      ...
      while (pFace != NULL) {
        if (!((pFace->GetRColor() == 0) && (pFace->GetGColor() == 0) &&
              (pFace->GetBColor() == 0))) {
        ...
        if (pFace->name != "") {
          sprintf(m_sOutput, "translate P%i_label P%i_%i P%i_%i P%i_%i",
                  m_ListOfFaces.GetGeometry(), m_ListOfFaces.GetGeometry(),
                  uFirstIndex, m_ListOfFaces.GetGeometry(), uFirstIndex,
        ...
        while (pVertex != NULL) {
          ...
          if (pFace->name != "") {
            sprintf(m_sOutput, "translate P%i_label P%i_%i P%i_%i P%i_label",
                    m_ListOfFaces.GetGeometry(), m_ListOfFaces.GetGeometry(),
                    uFirstIndex, m_ListOfFaces.GetGeometry(), pVertex->index,
        ...
        if (pFace->name != "") {
          sprintf(m_sOutput, "towards P%i_label P%i_%i P%i_label %3.2f",
                  m_ListOfFaces.GetGeometry(), m_ListOfFaces.GetGeometry(),
                  uFirstIndex, m_ListOfFaces.GetGeometry(),
                  (double)(1.0 / (iNumberOfVerteces + 1)));
          Output(m_sOutput);
...
GReturnValue CJavaView::ReadL() {
  ...
  sprintf(m_sOutput, "drawdashsegment P%i_%i P%i_%i", m_iGeometryIndex,
  ...
  if (substring != NULL) {
    ...
    sprintf(m_sOutput, "midpoint M%i_%i_%i_%i P%i_%i P%i_%i", m_iGeometryIndex,
            (int)(iFirst + 1), m_iGeometryIndex, (int)(iSecond + 1),
            m_iGeometryIndex, (int)(iFirst + 1), m_iGeometryIndex,
            (int)(iSecond + 1));
    Output(m_sOutput);
...
GReturnValue CJavaView::ReadV() {
  ...
  sprintf(m_sOutput, "point Q%i_%i %3.2f %3.2f", m_iGeometryIndex,
          m_iCurrentPoint, x + ZOOM * i, y + ZOOM * j);
  Output(m_sOutput);
...
GReturnValue CJavaView::ReadC() {
  ...
  m_ListOfPoints.AttachColorsToCurrentPoint((unsigned char)r, (unsigned char)g,
                                            (unsigned char)b);
...
GReturnValue CJavaView::ReadFaceC() {
  ...
  m_ListOfFaces.AttachColorsToCurrentFace((unsigned char)r, (unsigned char)g,
                                          (unsigned char)b);
...
JavaViewcommands CJavaView::choose_command(char *word, char *params) {
  ...
  for (i = 0; i < (int)_jv_number_of_commands - 1; i++) {
    ...
    if ((difflen >= -1) && (difflen <= 1)) {
      if (!strncmp(word, JVTagName[i][0], len) && word[len - 1] == '>')
