...
struct Constant {
  ...
  Constant(const string &name, int index) {
    Name = name;
    Index = index;
  }
...
struct Point {
  ...
  Point(bool xFree, bool yFree, const string &name) : X(xFree), Y(yFree) {
    Name = name;
  }
};
...
struct Line {
  ...
  vector<Point *> Points; // points on the line
  bool implicit;          // implicit line is line generated by the prover

  Line(const string &name) : p1(NULL), p2(NULL), Angle(-1) {
    Name = name;
    implicit = false;
  }
  ~Line() {}
...
struct Circle {
  ...
  Circle(const string &name, Point *cc, Point *pp) : c(cc), p(pp) {
    Name = name;
  }
  ~Circle() {}
};
...
struct Conic {
  ...
  Conic(const string &name) : H1(true), H2(true), H3(true), H4(true), H5(true) {
    Name = name;
  }
  ~Conic() {}
};
...
private:
  ...
  vector<CGCLCProverExpression> vpConjectures;
  vector<string> vsConjectures; // gclc conjecture string
  // original conjectures (in case they have changed)
  vector<CGCLCProverExpression> vpOrigConjectures;
  ...
  Point *_FindPoint(const string &name);
  Point *_FindOrAddPoint(const string &name);
  Line *_FindLine(const string &name);
  ...
  Constant *_FindConstant(const string &name);
  ...
  void _ReducibilityLineCircleIntersection(Point *p1, Point *p2, Line *l,
                                           Circle *k, Circle *k1);
  bool _UsedPoint(Point *p, list<CGCLCProverCommand>::iterator commandSkip);
...
public:
  ...
  bool AddProverCommand(eGCLC_prover_command type, const string &a1 = "",
                        const string &a2 = "", const string &a3 = "",
                        const string &a4 = "", const string &a5 = "");
  bool SetProverConjecture(const string &strConjecture);

  void SetPrintPoints(bool printPoints);
  void PrintMethodSpecificOutput();
  string GetMethodSpecificOutput();
  void PrintProofTitleXML(const string &theoremName);
